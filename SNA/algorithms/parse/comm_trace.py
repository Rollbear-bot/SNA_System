        ...
        for life_id, time_shot in enumerate(self._data):
            ...
            if "edgelist" in self._data[time_shot]:
                for edge in self._data[time_shot]["edgelist"]:
                    graph.add_edge(self._screen_name_map[edge[0]],
                                   self._screen_name_map[edge[1]],
                                   weight=edge[2])
            plt.clf()
            ...
            if len(graph.nodes) <= 50:
                ...
                nx.draw_networkx_labels(graph, pos, font_size=20)  # 画出点上的label
            ...
            dump_path = dump_dir + self.comm_id + "_" + str(time_shot) + ".png"
