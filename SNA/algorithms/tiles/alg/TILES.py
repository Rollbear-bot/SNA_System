...
class V211229DataConstructor:
    """211229新需求数据对象构造器"""

    def __init__(self, tiles_dump_dir):
        self._real_datetime_map = {}
        self._tile_dump_dir = tiles_dump_dir
        self._timeline_record = {}
        self._single_slice_comm_members = {}
        self._evo_record = []  # 需要有序，使用列表
        self._slice_count = 0

        self._get_slice_time_interval()

    def build_data(self):
        """
        需要全数据集所有社区的时间线，
        原有的CommTracer.dump_comm_meta_data_version2不再适用
        实现思路：
        只扫描edgelist一遍
        1. 在每个观测点先在strong_comm中得到当前生存的社区
        2. 在一个字典对象中注册当前时间片的所有社区
        3. 扫描一遍graph，得到当前时间片所有社区涉及的edge
        4. 向字典中社区对应处增加一条记录，即当前时间片的标识符和edges
        """
        cur_slice_id = 0
        pre_slice_comms = []  # 保存前一个时间片的社区ID

        while True:
            # print(f"cur slice: {cur_slice_id} / {self._slice_count}")
            try:
                # 获得当前时间片所有社区ID
                cur_comms = self._get_cur_slice_comm(cur_slice_id=cur_slice_id)
                # 在timeline记录对象中注册这些社区
                for comm_id in cur_comms:
                    if comm_id not in self._timeline_record:
                        self._timeline_record[comm_id] = {
                            "community_id": comm_id,
                            "timeline": []
                        }

                # 12月30日改动：输出edges改为输出nodes，降低时间复杂度
                # 在当前时间片获取这些社区的edges
                # cur_comm_edges = self._get_cur_slice_multi_comm_edgelist(cur_slice_id=cur_slice_id,
                #                                                          cur_slice_comm_data=cur_comms)
                # 向字典中社区对应处增加一条记录，即当前时间片的标识符和edges
                for comm_id, members in cur_comms.items():
                    self._timeline_record[comm_id]["timeline"].append({
                        "datetime": self._real_datetime_map[cur_slice_id]
                        if cur_slice_id in self._real_datetime_map else "unknown",
                        "nodes": members
                    })

                # 识别新生和消亡事件
                the_born = [comm_id for comm_id in cur_comms if comm_id not in pre_slice_comms] \
                    if cur_slice_id > 0 else []
                the_died = [comm_id for comm_id in pre_slice_comms if comm_id not in cur_comms] \
                    if cur_slice_id > 0 else []
                # 在演化记录对象中注册当前时间片
                self._evo_record.append((cur_slice_id, {
                    "datetime": self._real_datetime_map[cur_slice_id]
                    if cur_slice_id in self._real_datetime_map else "unknown",
                    "born": the_born,
                    "die": the_died,
                    "merge": self._get_cur_slice_merge(cur_slice_id),
                    "split": self._get_cur_slice_split(cur_slice_id)
                }))

                pre_slice_comms = cur_comms
                cur_slice_id += 1
        ...
        # 20220117改动：改为存储文件
        with open(self._tile_dump_dir + "_timeline.json", "w", encoding="utf8") as wf:
            json.dump(list(self._timeline_record.values()), wf, indent=2)
        with open(self._tile_dump_dir + "_evo.json", "w", encoding="utf8") as wf:
            json.dump([cluster[1] for cluster in self._evo_record], wf, indent=2)

        # return (list(self._timeline_record.values()),
        #         [cluster[1] for cluster in self._evo_record])

    def _get_cur_slice_comm(self, cur_slice_id):
        strong_comm_path = self._tile_dump_dir + f"strong-communities-{cur_slice_id}.gz"
        comm_dict = {}
        with gzip.open(strong_comm_path, 'rb') as f:
            ...
            for line in content.split("\n"):
                if line != "":
                    ...
                    comm_dict[arr[0]] = cur_members
        return comm_dict

    def _get_cur_slice_multi_comm_edgelist(self, cur_slice_id, cur_slice_comm_data):
        """返回一个字典{comm_id: [edgelist], ...}"""
        graph_path = self._tile_dump_dir + f"graph-{cur_slice_id}.gz"
        comm_edges = {comm_id: [] for comm_id in cur_slice_comm_data}

        with gzip.open(graph_path, "rb") as f:
            content = f.read().decode("utf8")
            for line in tqdm(list(content.split("\n")), postfix="getting cur slice edgelist..."):
                # 扫描一遍该时间片的所有边
                if line != "":
                    arr = line.split("\t")
                    source = arr[0]
                    ...
                    # 检查所有该时间片社区的成员，如果某边两端点都是社区的成员，则让这条边加入该社区的edgelist
                    for comm_id in comm_edges:
                        comm_members = cur_slice_comm_data[comm_id]
                        if source in comm_members and dest in comm_members:
                            comm_edges[comm_id].append((source, dest))
        return comm_edges

    def _get_slice_time_interval(self):
        pattern = re.compile(r'Saving Slice (\d+): Starting (\d+-\d+-\d+) \d+:\d+:\d+ ending \d+-\d+-\d+ \d+:\d+:\d+ -')
        res = pattern.findall(open(self._tile_dump_dir + "extraction_status.txt", "r").read())
        self._slice_count = len(res)
        for arr in res:
            self._real_datetime_map[int(arr[0])] = arr[1]

    def _get_cur_slice_merge(self, cur_slice_id):
        merging_path = self._tile_dump_dir + f"merging-{cur_slice_id}.gz"
        res = []
        with gzip.open(merging_path, "rb") as f:
            ...
            for line in content.split("\n"):
                if line != "":
                    arr = line.split("\t")
                    master = arr[0]  # 主社区
                    merged_lt = list(arr[1][1:-1].split(", "))  # 被合并的社区
                    res.append([master] + merged_lt)
        return res

    def _get_cur_slice_split(self, cur_slice_id):
        splitting_path = self._tile_dump_dir + f"splitting-{cur_slice_id}.gz"
        res = []
        with gzip.open(splitting_path, "rb") as f:
            ...
            for line in content.split("\n"):
                if line != "":
                    arr = line.split("\t")
                    splitting_comm = arr[0]  # 分裂的社区
                    new_comm_lt = list(arr[1][1:-1].split(", "))  # 形成的新社区
                    res.append([splitting_comm] + new_comm_lt)
        return res
...
class TILES(object):
    ...
    def execute(self):
        ...
        con = V211229DataConstructor(tiles_dump_dir=self.path)
        con.build_data()
